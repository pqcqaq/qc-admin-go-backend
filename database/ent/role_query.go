...
// QueryInheritedBy chains the current query on the "inherited_by" edge.
func (_q *RoleQuery) QueryInheritedBy() *RoleQuery {
...
// QueryInheritsFrom chains the current query on the "inherits_from" edge.
func (_q *RoleQuery) QueryInheritsFrom() *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// WithInheritedBy tells the query-builder to eager-load the nodes that are connected to
// the "inherited_by" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithInheritedBy(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInheritedBy = query
	return _q
}

// WithInheritsFrom tells the query-builder to eager-load the nodes that are connected to
// the "inherits_from" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithInheritsFrom(opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withInheritedBy; query != nil {
		if err := _q.loadInheritedBy(ctx, query, nodes,
			func(n *Role) { n.Edges.InheritedBy = []*Role{} },
			func(n *Role, e *Role) { n.Edges.InheritedBy = append(n.Edges.InheritedBy, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInheritsFrom; query != nil {
...
func (_q *RoleQuery) loadInheritedBy(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.InheritedByTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(role.InheritedByPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.InheritedByPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.InheritedByPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "inherited_by" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *RoleQuery) loadInheritsFrom(ctx context.Context, query *RoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *Role)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Role)
	nids := make(map[uint64]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.InheritsFromTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(role.InheritsFromPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.InheritsFromPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.InheritsFromPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "inherits_from" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
