...
func (_q *StationQuery) QueryArea() *AreaQuery {
...
func (_q *StationQuery) QuerySubwayStations() *SubwayStationQuery {
...
func (_q *StationQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *StationQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *StationQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *StationQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *StationQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *StationQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *StationQuery) WithArea(opts ...func(*AreaQuery)) *StationQuery {
	query := (&AreaClient{config: _q.config}).Query()
...
func (_q *StationQuery) WithSubwayStations(opts ...func(*SubwayStationQuery)) *StationQuery {
	query := (&SubwayStationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(station.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Station.Query().
//		Select(station.FieldCreateTime).
//		Scan(ctx, &v)
...
func (_q *StationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Station, error) {
	...
	if query := _q.withArea; query != nil {
		if err := _q.loadArea(ctx, query, nodes, nil,
			func(n *Station, e *Area) { n.Edges.Area = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubwayStations; query != nil {
		if err := _q.loadSubwayStations(ctx, query, nodes,
			func(n *Station) { n.Edges.SubwayStations = []*SubwayStation{} },
			func(n *Station, e *SubwayStation) { n.Edges.SubwayStations = append(n.Edges.SubwayStations, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedSubwayStations {
		if err := _q.loadSubwayStations(ctx, query, nodes,
			func(n *Station) { n.appendNamedSubwayStations(name) },
			func(n *Station, e *SubwayStation) { n.appendNamedSubwayStations(name, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StationQuery) loadArea(ctx context.Context, query *AreaQuery, nodes []*Station, init func(*Station), assign func(*Station, *Area)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Station)
	for i := range nodes {
		fk := nodes[i].AreaID
...
func (_q *StationQuery) loadSubwayStations(ctx context.Context, query *SubwayStationQuery, nodes []*Station, init func(*Station), assign func(*Station, *SubwayStation)) error {
	...
	query.Where(predicate.SubwayStation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(station.SubwayStationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.station_subway_stations
		if fk == nil {
			return fmt.Errorf(`foreign-key "station_subway_stations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "station_subway_stations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *StationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(station.Table, station.Columns, sqlgraph.NewFieldSpec(station.FieldID, field.TypeUint64))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withArea != nil {
			_spec.Node.AddColumnOnce(station.FieldAreaID)
		}
	}
...
func (_q *StationQuery) ForUpdate(opts ...sql.LockOption) *StationQuery {
...
func (_q *StationQuery) ForShare(opts ...sql.LockOption) *StationQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
...
func (_q *StationQuery) WithNamedSubwayStations(name string, opts ...func(*SubwayStationQuery)) *StationQuery {
	query := (&SubwayStationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
