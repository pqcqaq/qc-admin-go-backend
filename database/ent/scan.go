...
func (e ScanEdges) AttachmentOrErr() (*Attachment, error) {
	if e.Attachment != nil {
...
func (*Scan) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case scan.FieldID, scan.FieldCreateBy, scan.FieldUpdateBy, scan.FieldDeleteBy, scan.FieldLength:
			values[i] = new(sql.NullInt64)
		case scan.FieldContent:
			values[i] = new(sql.NullString)
		case scan.FieldCreateTime, scan.FieldUpdateTime, scan.FieldDeleteTime:
			values[i] = new(sql.NullTime)
		case scan.ForeignKeys[0]: // scan_attachment
			values[i] = new(sql.NullInt64)
		default:
...
func (_m *Scan) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case scan.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field scan_attachment", values[i])
			} else if value.Valid {
				_m.scan_attachment = new(uint64)
				*_m.scan_attachment = uint64(value.Int64)
			}
		default:
...
// QueryAttachment queries the "attachment" edge of the Scan entity.
func (_m *Scan) QueryAttachment() *AttachmentQuery {
	return NewScanClient(_m.config).QueryAttachment(_m)
}
