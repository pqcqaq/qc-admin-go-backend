...
func (_q *ScanQuery) QueryAttachment() *AttachmentQuery {
...
func (_q *ScanQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ScanQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *ScanQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ScanQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *ScanQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *ScanQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ScanQuery) WithAttachment(opts ...func(*AttachmentQuery)) *ScanQuery {
	query := (&AttachmentClient{config: _q.config}).Query()
...
func (_q *ScanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Scan, error) {
	...
	if query := _q.withAttachment; query != nil {
		if err := _q.loadAttachment(ctx, query, nodes, nil,
			func(n *Scan, e *Attachment) { n.Edges.Attachment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ScanQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*Scan, init func(*Scan), assign func(*Scan, *Attachment)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Scan)
	for i := range nodes {
		if nodes[i].scan_attachment == nil {
			continue
		}
		fk := *nodes[i].scan_attachment
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "scan_attachment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ScanQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(scan.Table, scan.Columns, sqlgraph.NewFieldSpec(scan.FieldID, field.TypeUint64))
	_spec.From = _q.sql
...
func (_q *ScanQuery) ForUpdate(opts ...sql.LockOption) *ScanQuery {
...
func (_q *ScanQuery) ForShare(opts ...sql.LockOption) *ScanQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
